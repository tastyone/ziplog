...
/**
 * @method stats_visitors
 * @param {String} hash
 * @param {String} from date formated 'YYYY-MM-DD'
 * @param {String} to date formated 'YYYY-MM-DD'
 * @param {Function} callback 
 */
var stats_visitors = exports.stats_visitors = function(p_hash, p_from, p_to, callback) {
  var d_start = new Date(p_from);
  var d_until = new Date(p_to).addDays(1);
  var uv = undefined;
  var pv = undefined;
  var q = {hash: p_hash, created_at: {"$gte": d_start, "$lt": d_until}};
  ZipLogArchive.count(q, function(error, count) {
    pv = count;
    if ( uv != undefined ) {
      callback(error, {pv: pv, uv: uv});
    }
  });
  ZipLogArchive.distinct('cookie', q).exec(function(error, cookies) {
    uv = cookies.length;
    if ( pv != undefined ) {
      callback(error, {pv: pv, uv: uv});
    }
